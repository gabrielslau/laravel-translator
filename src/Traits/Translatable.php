...
namespace Okatsuralau\LaravelTranslator\Traits;

use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\Relations\Relation;
use Okatsuralau\LaravelTranslator\LaravelTranslator as Translator;
use Okatsuralau\LaravelTranslator\Models\Translation;
...
trait Translatable
{
    ...
    public function getTranslationsOf($attribute, array $languages = null, $fallback = true)
    {
        if (is_null($languages)) {
            $languages = config('app.locales', [config('app.default_locale')]);
        }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('app.default_locale'), false];
        }
    ...
    public function setAttributeTranslations($attribute, array $translations, $save = false)
    {
        ...
        $default = config('app.default_locale', 'pt');
