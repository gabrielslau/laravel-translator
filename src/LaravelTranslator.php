...
class LaravelTranslator implements ArrayAccess, JsonSerializable
{
    protected $model;
    ...
    public function __construct(Model $model)
    {
        ...
        $this->model = $model;
        $this->locale = config('app.default_locale', 'pt');
        $attributes = [];
    ...
    protected function translateAttributeToOriginal($attribute)
    {
        $this->attributes[$attribute] = [
            ...
            'locale'   => config('app.default_locale', 'pt'),
            'exists'   => true,
            'modified' => false,
        ];
    ...
    public function __call($method, array $arguments)
    {
        if (!$this->model->hasTranslatorMethod($method)) {
            throw new \Exception('Call to undefined method Okatsuralau\LaravelTranslator\LaravelTranslator::'.$method.'()');
        }
